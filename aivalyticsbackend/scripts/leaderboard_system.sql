...
-- This script adds automatic leaderboard calculation functionality

-- Step 1: Add leaderboard fields to the user table
ALTER TABLE "user" ADD COLUMN IF NOT EXISTS total_score INTEGER DEFAULT 0;
ALTER TABLE "user" ADD COLUMN IF NOT EXISTS total_quizzes_taken INTEGER DEFAULT 0;
ALTER TABLE "user" ADD COLUMN IF NOT EXISTS average_score DECIMAL(5,2) DEFAULT 0.00;
ALTER TABLE "user" ADD COLUMN IF NOT EXISTS highest_score INTEGER DEFAULT 0;
ALTER TABLE "user" ADD COLUMN IF NOT EXISTS overall_percentage DECIMAL(5,2) DEFAULT 0.00;
...
DECLARE
    ...
    user_overall_percentage DECIMAL(5,2);
    user_leaderboard_points INTEGER;
    user_total_marks INTEGER;
BEGIN
    -- Calculate statistics from score table
    SELECT 
        COALESCE(SUM(marks_obtained), 0) as total_score,
        COALESCE(COUNT(*), 0) as quiz_count,
        COALESCE(AVG(marks_obtained), 0) as avg_score,
        COALESCE(MAX(marks_obtained), 0) as highest_score,
    ...
    IF user_total_marks > 0 THEN
        user_overall_percentage = (user_total_score::DECIMAL / user_total_marks::DECIMAL) * 100;
    ELSE
...
COMMENT ON COLUMN "user".total_score IS 'Sum of all marks obtained by the user across all quizzes';
COMMENT ON COLUMN "user".total_quizzes_taken IS 'Total number of quizzes attempted by the user';
COMMENT ON COLUMN "user".average_score IS 'Average marks obtained per quiz';
COMMENT ON COLUMN "user".highest_score IS 'Highest marks obtained in a single quiz';
COMMENT ON COLUMN "user".overall_percentage IS 'Overall percentage across all quizzes';
...
COMMENT ON VIEW leaderboard_view IS 'Materialized view showing current leaderboard rankings with all relevant statistics';
...
-- Step 10: Create function to get top N leaderboard entries
...
BEGIN
    ...
    RAISE NOTICE '📊 Added leaderboard fields to user table';
